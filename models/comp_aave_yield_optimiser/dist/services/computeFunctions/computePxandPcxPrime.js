...
import { calcAaveSupplyRate, calcAaveSupplyRateDerivative, calcAaveSupplyRateSecondDerivative, } from "../aave/services.js";
import { rayMul } from "../../utils/mathUtils.js";
import { calcCompSupplyRate, calcCompSupplyRateDerivative, calcCompSupplyRateSecondDerivative, } from "../compound/services.js";
/**
 ...
 */
export function computePxAndPxPrimeDepositOrRebalance(params) {
    ...
    const aaveSupplyRateDerivative = calcAaveSupplyRateDerivative(aaveBalanceDelta, aaveConstants);
    const aaveSupplyRateSecondDerivative = calcAaveSupplyRateSecondDerivative(aaveBalanceDelta, aaveConstants);
    const compBalanceDelta = W - x;
    const compSupplyRate = wadToRay(calcCompSupplyRate(compBalanceDelta, compConstants));
    const compSupplyRateDerivative = wadToRay(calcCompSupplyRateDerivative(compBalanceDelta, compConstants));
    const compSupplyRateSecondDerivative = wadToRay(calcCompSupplyRateSecondDerivative(compBalanceDelta, compConstants));
    ...
    // return fx;
    //P'(x) =2U'(x) + 2K'(W-x) + (A + x)U"(x)  + (C + W -x)K"(W-x)
    const pxPrime = 2n * rateDerivativeSum +
 ...
 * @returns  [P(x),P'(x)] where both values are in Ray factor
 */
export function computePxAndPxPrimeWithdrawal(params) {
    const { A, C, W, x, aaveConstants, compConstants, decimals } = params;
    const aaveBalanceDelta = -x;
    const aaveSupplyRate = calcAaveSupplyRate(aaveBalanceDelta, aaveConstants);
    const aaveSupplyRateDerivative = calcAaveSupplyRateDerivative(aaveBalanceDelta, aaveConstants);
    ...
    const fxPrime = 2n * rateDerivativeSum +
        rayMul(netBalanceAfterAave, aaveSupplyRateSecondDerivative) +
        rayMul(netBalanceAfterComp, compSupplyRateSecondDerivative);
    return [fx, fxPrime];
}
//# sourceMappingURL=computePxandPcxPrime.js.map