...
//     return balance;
//   }
export async function supplyToCompound(assetMetaData, amount, pkey) {
...
export async function withdrawFromCompound(assetMetaData, amount, pkey) {
...
export async function getDetailsCompound(assetMetaData) {
    ...
    try {
        const [availableLiquidity, totalBorrow, totalSupply, supplyKink, supplyPerSecondInterestRateBase, supplyPerSecondInterestRateSlopeLow, supplyPerSecondInterestRateSlopeHigh,] = (await Promise.all(promises));
        return {
            decimals,
            totalBorrow,
            totalSupply,
