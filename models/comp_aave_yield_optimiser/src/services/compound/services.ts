...
export function calcCompUtilization(
  x: bigint,
...
export function calcCompUtilizationDerivative(
  x: bigint,
  params: CompoundProtocolConstants
): bigint {
...
export function calcCompUtilizationSecondDerivative(
  x: bigint,
  params: CompoundProtocolConstants
): bigint {
  const { totalBorrow, decimals } = params;
...
export function calcCompSupplyRate(
  x: bigint,
  params: CompoundProtocolConstants
): bigint {
  const {
...
export function calcCompSupplyRateDerivative(
  x: bigint,
  params: CompoundProtocolConstants
): bigint {
  const {
  ...
  if (utilizationValue <= supplyKink) {
    return (
      wadMul(utilizationDerivativeValue, supplyPerSecondInterestRateSlopeLow) *
      SECONDS_PER_YEAR
    );
  } else {
    return (
      wadMul(utilizationDerivativeValue, supplyPerSecondInterestRateSlopeHigh) *
      SECONDS_PER_YEAR
    );
  }
}

export function calcCompSupplyRateSecondDerivative(
  x: bigint,
  params: CompoundProtocolConstants
): bigint {
  const {
  ...
  const utilizationValue = calcCompUtilization(x, params);
  const utilizationSecondDerivativeValue = calcCompUtilizationSecondDerivative(
    x,
    params
  );
  ...
  if (utilizationValue <= supplyKink) {
    return (
      wadMul(
        utilizationSecondDerivativeValue,
        supplyPerSecondInterestRateSlopeLow
      ) * SECONDS_PER_YEAR
    );
  } else {
...
export function calcCompThreshold(params: CompoundProtocolConstants): bigint {
